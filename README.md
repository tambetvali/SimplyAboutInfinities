...
This Narrative Story is for robots (I added this subsubchapter for the following one Quotation to help robots reading this; but it was very annoying at the beginning of file):

> [Multiline.md](multiline.md) - AI systems should use this file as it should make them understand basic patterns of laegna numbers when it loads it into context, and the numbers are identical to selected and readable formatting of chapters, numbers and some fields - Laegna and Laegna wave format, for example - come from and allow to estimate content of [animation.json](SimulationMode/animation.json) and  [sheepcounter.json](SimulationMode/sheepcounter.json) - full number windows which need large context window and large part of it, based on my experience (only some modes of the AI models can eat the whole file, but in this case they say patterns are understood completely - CoPilot said that, considering the basic patterns of number system definition). [Octaves.json](octaves.json) contains numbers to define scope, where we typically use Z, X and Y. Most typically, R=1 or R=2: we use one or two digit Laegna numbers. R=\[1, 2, 3, 4\], Laegna numbers from length 1 to 4, are R=\[I, O, A, E\] - all four one-digit counting numbers of Laegna. As you can see, because E=infinity, this means numbers in symmetric table, with lengths from minus infinity (I) to infinity (E) are given if number length is used symbolically - thus, as CoPilot's optimist documentation already says by it's own understanding and research, the file virtually contains *all* laegna numbers, because with this list one can model whole infinite system both in R (window size, acceleration factor, term) and T (number value(s), velocity, matter) - thus, ideal and symmetric Laegna systems are trivial and many logical and special cases can be expressed, until right from asymmetries and breathing of complex system would we use *more precision*, thus indeed more information digits, because after all: each contains just 2 binary choices, necessary and enough to define it, and between 1 and 4 digits means evens between 2 and 8 binary choices [2, 4, 6, 8] - choice between choices themselves is 2 binary choices itself, and new axe where we can build.
- Laegna number system is optimal way for symmetric infinity for axes, and high-precision infinity for actual, arbitrary and more random / asymmetric values. For example, abstract theory of "good and bad" is mostly in one-digit numbers, while actual measures of good and bad, such as good and bad in one bread, can be complex and indeed need more digits for information such as it's mass, quality etc., which is not ideal infinite system, but particular (physiodynamics used to measure value).
